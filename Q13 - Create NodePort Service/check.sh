...
check_criterion "Service api-nodeport exists in q13" \
  "$(resource_exists svc/api-nodeport q13)" && ((score++))

svc_type=$(kget svc/api-nodeport q13 '.spec.type')
check_criterion "Service type is NodePort" \
  "$([ "$svc_type" = "NodePort" ] && echo true || echo false)" && ((score++))

svc_sel=$(kget svc/api-nodeport q13 '.spec.selector.app')
check_criterion "Selector is app=api" \
  "$([ "$svc_sel" = "api" ] && echo true || echo false)" && ((score++))

svc_port=$(kget svc/api-nodeport q13 '.spec.ports[0].port')
tgt_port=$(kget svc/api-nodeport q13 '.spec.ports[0].targetPort')
